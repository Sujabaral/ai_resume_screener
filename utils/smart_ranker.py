...
def bert_similarity(a, b):
    ...
    try:
        emb_a = MODEL.encode(a[:6000], convert_to_tensor=True)
        emb_b = MODEL.encode(b[:6000], convert_to_tensor=True)
...
def skill_evidence(cv_text, required_skills, preferred_skills):
    ...
    return {
        "required_score": round((total / max_total) * 100, 2),
        "preferred_score": round((preferred_total / preferred_max) * 100, 2) if preferred_skills else 100,
...
def qualification_score(cv_text, required_qualification):
    ...
    if required_level and candidate_level == 0:
        return 45, f"Required qualification not clearly found: {required_qualification}."

    if required_level and candidate_level < required_level:
        return 65, f"Candidate qualification may be below requirement: {required_qualification}."
...
def structure_score(cv_text):
    ...
    important = ["skills", "technical skills", "experience", "work experience", "education", "projects"]
    found = [s for s in important if len(sections.get(s, "").strip()) > 20]

    return min(100, round((len(found) / 5) * 100, 2)), found
...
def get_confidence_level(overall, semantic_score, required_missing_count):
    if overall >= 80 and semantic_score >= 70 and required_missing_count <= 1:
        return "High Confidence"
    elif overall >= 60 and semantic_score >= 50:
        return "Medium Confidence"
    return "Low Confidence"


def get_hr_recommendation(overall, missing_required, experience_score):
    if overall >= 80 and not missing_required and experience_score >= 80:
...
def get_hr_recommendation(overall, missing_required, experience_score):
    if overall >= 80 and not missing_required and experience_score >= 75:
        return "Recommended for Interview"
    elif overall >= 60:
...
def build_deep_analysis(data):
    ...
    elif data["responsibility_score"] < 50:
        risks.append("Job responsibility evidence is weak or unclear.")

    if data["candidate_years"] >= data["required_years"]:
...
HR Note:
This is an AI-assisted screening result only. 
""".strip()

def rank_candidate(cv_text, jd_data):
    required_skills = jd_data.get("required_skills", [])
    preferred_skills = jd_data.get("preferred_skills", [])
    required_years = jd_data.get("required_years", 0)
    required_qualification = jd_data.get("qualification", "")
    ...
    responsibility_score = bert_similarity(cv_text, responsibilities_text) if responsibilities_text else semantic_score

    education_score, education_note = qualification_score(cv_text, required_qualification)

    project_score = best_semantic_match_score(
        cv_text,
        job_description
    ...
    elif required_years and candidate_years > 0:
        experience_score = round((candidate_years / required_years) * 100, 2)
